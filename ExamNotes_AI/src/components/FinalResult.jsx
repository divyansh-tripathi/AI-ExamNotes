...
const markDownComponent = {
  ...
  h3: ({ children }) => (
    <h3 className="text-xl font-bold text-indigo-700 mt-4 mb-2"> {children}</h3>
  ),
  p: ({ children }) => (
...
const FinalResult = ({ result }) => {
  ...
  if (!result || !result.subTopics || !result.questions) {
  return null;
}
  return (
    <div className="mt-6 p-3 space-y-10 bg-white">
      ...
      {!quickRevision && (
        <section>
          <SelectionHeader icon="⭐" title="Sub Topics" color="indigo" />
      ...
      {!quickRevision && (
        <section>
          <SelectionHeader icon="📝" title="Detailed Notes" color="purple" />
      ...
      {quickRevision && (
        <section className="rounded-xl bg-gradient-to-r from-green-100 to-green-50 border border-green-200 p-6">
          ...
          <ul className="list-disc ml-6 space-y-1 text-gray-800">
            {(result.revisionPoints || result.revisionPoint || []).map(
              (p, i) => (
                <li key={i}>{p}</li>
              ),
            )}
      ...
      {result.diagram?.data && (
        <section>
          <SelectionHeader icon="📊" title="Diagram" color="cyan" />
      ...
      {result.charts?.length > 0 && (
        <section>
          ...
          <SelectionHeader icon="📈" title="Visual Charts" color="indigo" />
      ...
      <section className="space-y-4">
        <SelectionHeader icon="❓" title="Important Questions" color="rose" />
        ...
        {result.questions.long && result.questions.long.length > 0 && (
          <div>
            <p className="font-semibold text-gray-800">Long Questions:</p>
            <ul className="list-disc ml-6 space-y-1 text-gray-700">
        ...
        {result.questions.diagram && (
          <div>
            <p className="font-semibold text-gray-800">Diagram Question:</p>
            <ul className="list-disc ml-6 space-y-1 text-gray-700">
...
function SelectionHeader({ icon, title, color }) {
  const colors = {
  ...
  return (
    <div
      className={`mb-4 py-2 px-4 rounded-lg bg-gradient-to-r ${colors[color]} font-semibold flex items-center gap-2`}
    >
      <span>{icon}</span>
      <span>{title}</span>
    </div>
