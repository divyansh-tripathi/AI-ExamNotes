...
const TopicForm = (props) => {
  const { setLoading, setResult, loading, setError, error } = props.values;
  // console.log(error)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(error);
      setError("Failed a fetch notes from server.");
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <input
        type="text"
        className="w-full p-3 rounded-xl bg-white/10 backfrop-blue-lg border border-white/20 placeholder-gray-400 text-white focus:outline-none focus:ring-2 focus:ring-white/30"
      ...
      <motion.button
        ...
        className={`w-full mt-4 py-3 rounded-xl font-semibold flex items-center justify-center gap-3 transition ${loading ? "bg-gray-300 text-gray-600 cursor-not-allowed" : "bg-gradient-to-br from-white to-gray-100 text-black shadow-[0_15px_35px_rgba(0,0,0,0.2)]"}`}
      >
      ...
      {loading && (
        <div className="mt-4 space-y-2">
          ...
          <p className="text-xs text-gray-400 text-center">
            This may takes up to 2-5 minutes. Please don't close or refresh the
            page.
          </p>
        </div>
      ) }
